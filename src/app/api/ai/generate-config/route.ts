...
/* ---------- Linting dasar per vendor (bahan §6) ---------- */
function lintConfig(
  node: { type: string; vendor: string; interfaces?: { ip: string }[] } | undefined,
  script: string
  ...
  if (node.type === "pc" || node.type === "laptop" || node.type === "ap" || node.type === "cloud" || node.type === "printer") {
    return { isValid: true, issues: [] }; // tidak butuh config
  }
  ...
  switch (node.vendor) {
    ...
    case "cisco": {
      const opens = lines.filter((l) => l.startsWith("interface ") || l.startsWith("router ")).length;
